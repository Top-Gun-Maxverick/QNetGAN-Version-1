...
def train_val_test_split_adjacency(A, p_val=0.10, p_test=0.05, seed=0, neg_mul=1,
    ...
    #print(f"Undirected is {undirected}")
    #print(f"is_undirected is {is_undirected}")
    #print(A)

    if undirected:
        assert is_undirected  # make sure is directed
        #print("Oh wow the graph is undirected!")
        try: A = sp.tril(A).tocsr()  # consider only upper triangular
        except TypeError: print("oh noes the fucking code is shit!")
        A.eliminate_zeros()
    else:
        if is_undirected:
    ...
    print("This graph is" + (" " if connected else "not ") + "connected") #debug 
    if every_node:
        if connected:
    ...
    print(f"idx_ones looks like {idx_ones}") #debug
    train_ones = ones[idx_ones[:s_train]]
    test_ones = ones[idx_ones[s_train:]]
    print(f"s_train is equal to {s_train}") #debug
    print(f"test_ones looks like {test_ones}") #debug
    ...
    print(f"N_test is {n_test}.") #debug
    if set_ops:
    ...
    print(f"test_zeros looks like {test_zeros} with length {len(test_zeros)}.")
    ...
    if undirected: #what if i dont do this
