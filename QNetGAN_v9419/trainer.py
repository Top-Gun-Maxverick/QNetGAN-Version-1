...
#I HAVEN'T REVIEWED THIS CODE YET WILL DO SOON
class Trainer():
    def __init__(self, graph,  N, max_iterations=20000, rw_len=16, batch_size=128, H_gen=40, H_disc=30, H_inp=128, z_dim=16, lr=0.0003, n_critic=3, gp_weight=10.0, betas=(.5, .9),
        ...
        #self.walker = stuff[6]
        self.train_ones, self.val_ones, self.val_zeros, self.test_ones, self.test_zeros = utils.train_val_test_split_adjacency(graph, val_share, test_share, seed, undirected=self.undir, connected=True, asserts=True, set_ops=set_ops)
